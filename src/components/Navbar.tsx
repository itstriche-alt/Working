    ...
    <>
      <nav
        className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 border-b ${scrolled
          ? 'bg-white/90 backdrop-blur-sm md:backdrop-blur-md border-slate-200 py-3 shadow-sm'
      ...
      <AnimatePresence>
        {isOpen && (
          <m.div
            ...
            className="fixed inset-0 z-40 bg-white/95 backdrop-blur-md lg:hidden pt-28 px-6 pb-10 flex flex-col overflow-y-auto"
          >
              ...
              {navLinks.map((link, idx) => (
                ...
                >
                  <Link
                    ...
                    className="text-3xl font-bold text-slate-900 py-4 border-b border-slate-100 hover:text-orange-800 hover:pl-4 transition-all flex items-center justify-between group cursor-pointer block w-full"
                  >
                    {link.name}
                    <ICONS.ArrowRight className="w-6 h-6 opacity-0 group-hover:opacity-100 -translate-x-4 group-hover:translate-x-0 transition-all text-orange-800" />
                  </Link>
                </m.div>
              ))}
            </div>
            ...
            >
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                >
                  <ICONS.Mail className="w-6 h-6 text-orange-800" />
                  <span>Email Us</span>
                </Link>
